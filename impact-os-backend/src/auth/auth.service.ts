...
// import * as crypto from 'crypto'; // Removed manual crypto
...
export class AuthService {
  ...
  async requestOtp(
    ...
    if (!user) {
      ...
      this.logger.warn(
        `OTP requested for non-existent email: ${normalizedEmail}`,
      );
    ...
    if (isDev) {
      ...
      this.logger.log(
        `[DEV MODE] Use code "000000" to bypass OTP verification`,
      );
      return {
        success: true,
  ...
  async verifyOtp(
    ...
    const isDev = process.env.NODE_ENV !== 'production';
    const isValidOtp =
      (isDev && code === '000000') || (user.otpCode && user.otpCode === code);
  ...
  async loginByUsernamePin(
    ...
    if (!user.isActive) {
      throw new UnauthorizedException(
        'Account is paused. Please contact support.',
      );
    }
