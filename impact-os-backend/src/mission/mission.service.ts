...
export class MissionService {
  ...
  async completeMission(
    ...
    // Distribute rewards
    await this.currencyService.rewardMission(userId, assignment.missionId, {
  ...
  async assignDailyMissions(): Promise<number> {
    ...
    for (const user of activeUsers) {
      ...
      const eligibleMissions = dailyMissions.filter((m) => {
        ...
        return (
          levelOrder.indexOf(m.requiredLevel) <=
          levelOrder.indexOf(user.identityLevel)
        );
      });
      ...
      const randomMission =
        eligibleMissions[Math.floor(Math.random() * eligibleMissions.length)];
    ...
    const missionsWithStats = await Promise.all(
      missions.map(async (mission) => {
        ...
        const completedCount =
          assignments.find((a) => a.status === MissionStatus.VERIFIED)
            ?._count || 0;
  ...
  async getMissionStats() {
    const [totalMissions, activeMissions, totalAssignments, statusCounts] =
      await Promise.all([
    ...
    const pendingReview =
      statusCounts.find((s) => s.status === MissionStatus.SUBMITTED)?._count ||
      0;
    const completed =
      statusCounts.find((s) => s.status === MissionStatus.VERIFIED)?._count ||
      0;
    const inProgress = statusCounts
      .filter((s) =>
        (
          [MissionStatus.ASSIGNED, MissionStatus.IN_PROGRESS] as MissionStatus[]
        ).includes(s.status),
      )
