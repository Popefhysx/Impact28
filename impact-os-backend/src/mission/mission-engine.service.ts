...
import { AlertType, AlertSeverity, CurrencyType, MissionStatus, SkillDomain } from '@prisma/client';
...
export class MissionEngineService {
    ...
    async checkTriadImbalance(userId: string) {
        ...
        if (max > 50 && min < this.THRESHOLDS.TRIAD_IMBALANCE) {
            const lowDomain = score.technical < this.THRESHOLDS.TRIAD_IMBALANCE ? 'Technical' :
                score.soft < this.THRESHOLDS.TRIAD_IMBALANCE ? 'Soft' : 'Commercial';
    ...
    async applyMomentumDecay() {
        ...
        this.logger.log(`Momentum decay: ${results.decayed} users decayed, ${results.alertsCreated} alerts`);
        return results;
    }
    ...
    async checkMissedMissions() {
        ...
        for (const user of usersWithMissed) {
            await this.createInterventionAlert(user.userId, AlertType.MISSED_MISSIONS, {
                severity: AlertSeverity.HIGH,
    ...
    async processCheckIn(userId: string) {
        ...
        const isConsecutive = user.lastCheckIn &&
            this.isConsecutiveDay(user.lastCheckIn, today);
    ...
    async recalculateWallRanks() {
        ...
        for (const post of posts) {
            ...
            const rankScore = triadBalance + balance.momentum + streakBonus + arenaBonus + hashtagBonus + recencyBonus;
    ...
    private getRecencyScore(submittedAt: Date): number {
        const daysSincePost = (Date.now() - submittedAt.getTime()) / (1000 * 60 * 60 * 24);
     ...
     */
    async logIdentityUpgrade(userId: string, previousLevel: string, newLevel: string, reason: string) {
        if (previousLevel === newLevel) return;

        this.logger.log(`Identity upgrade for ${userId}: ${previousLevel} -> ${newLevel} (${reason})`);
