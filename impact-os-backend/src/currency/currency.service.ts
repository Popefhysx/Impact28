...
export class CurrencyService {
    ...
    async credit(
        ...
        this.logger.log(`Credited ${amount} ${currencyType} to user ${userId}: ${reason}`);
    ...
    async debit(
        ...
        if (currentBalance < amount) {
            this.logger.warn(`Insufficient ${currencyType} balance for user ${userId}`);
        ...
        this.logger.log(`Debited ${amount} ${currencyType} from user ${userId}: ${reason}`);
     ...
     */
    private async getBalanceForType(userId: string, currencyType: CurrencyType): Promise<number> {
     ...
     */
    async applyMomentumDecay(userId: string, decayRate: number = 0.1): Promise<void> {
        const currentMomentum = await this.getBalanceForType(userId, CurrencyType.MOMENTUM);

        if (currentMomentum <= 0) return;

        const decayAmount = Math.ceil(currentMomentum * decayRate);

        await this.debit(userId, CurrencyType.MOMENTUM, decayAmount, 'Momentum decay (inactivity)');

        this.logger.log(`Applied momentum decay of ${decayAmount} to user ${userId}`);
    }
    ...
    async rewardMission(
        ...
        if (rewards.momentum && rewards.momentum > 0) {
            await this.credit(userId, CurrencyType.MOMENTUM, rewards.momentum, reason, missionId);
        }

        if (rewards.skillXp && rewards.skillXp > 0) {
            await this.credit(userId, CurrencyType.SKILL_XP, rewards.skillXp, reason, missionId);
        }

        if (rewards.arenaPoints && rewards.arenaPoints > 0) {
            await this.credit(userId, CurrencyType.ARENA_POINTS, rewards.arenaPoints, reason, missionId);
        }
    }
     ...
     */
    async canAfford(userId: string, currencyType: CurrencyType, cost: number): Promise<boolean> {
        const balance = await this.getBalanceForType(userId, currencyType);
        return balance >= cost;
    }
}
