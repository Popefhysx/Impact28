...
export class ProgressService {
  ...
  async getDashboardProgress(userId: string): Promise<DashboardProgress> {
    ...
    return {
      ...
      kpiTargets:
        (user.applicant?.kpiTargets as DashboardProgress['kpiTargets']) ?? null,
      phase: phaseInfo,
      upcomingEvents,
    };
  }
   ...
   */
  private async calculateCurrentPhase(
    daysInProgram: number,
  ): Promise<DashboardProgress['phase']> {
    ...
    for (const phase of phases) {
      ...
      if (daysInProgram < cumulativeDays) {
        const daysIntoPhase =
          daysInProgram - (cumulativeDays - phase.durationDays);
        return {
          current: phase.slug,
          name: phase.name,
          order: phase.order,
  ...
  ): Promise<DashboardProgress['upcomingEvents']> {
    const now = new Date();
   ...
   */
  private async assignStarterMissions(
    userId: string,
    skillTrack: string,
  ): Promise<void> {
    ...
    for (const mission of starterMissions) {
      try {
        ...
        this.logger.warn(
          `Failed to assign mission ${mission.id} to user ${userId}`,
        );
      }
    }

    this.logger.log(
      `Assigned ${starterMissions.length} starter missions to user ${userId}`,
    );
  }
