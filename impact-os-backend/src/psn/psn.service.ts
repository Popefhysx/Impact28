...
export class PsnService {
  ...
  async generateCohortForecast(
    ...
    // Count distribution
    const countHigh = refreshedApplicants.filter(
      (a) => a.psnLevel === PsnLevel.HIGH,
    ).length;
    const countMedium = refreshedApplicants.filter(
      (a) => a.psnLevel === PsnLevel.MEDIUM,
    ).length;
    const countLow = refreshedApplicants.filter(
      (a) => a.psnLevel === PsnLevel.LOW,
    ).length;

    // Calculate demand predictions (in USD)
    // Base allocation: $100 per participant (MSA)
    const MSA = 100;
    const highUtilization = 0.8; // 80% of budget used
    const mediumUtilization = 0.5;
    const lowUtilization = 0.2;
    ...
    if (countHigh / refreshedApplicants.length > 0.4) {
      ...
      countHigh / refreshedApplicants.length > 0.25
    ) {
  ...
  async runBehavioralTriggerDetection(): Promise<{
    ...
    this.logger.log(
      `Behavioral trigger detection complete: ${triggersCreated} triggers for ${participants.length} participants`,
    );
   ...
   */
  private async detectMomentumDrop(
    participantId: string,
  ): Promise<{ signalType: string; severity: DetectionSeverity } | null> {
    ...
    const netChange = momentumLedger.reduce(
    ...
    if (netChange <= -20) {
      return {
        ...
        severity:
          netChange <= -40 ? DetectionSeverity.HIGH : DetectionSeverity.MEDIUM,
      };
    }
   ...
   */
  private async detectPhaseBlocker(
    participantId: string,
  ): Promise<{ signalType: string; severity: DetectionSeverity } | null> {
   ...
   */
  private async detectRepeatedNearMisses(
    participantId: string,
  ): Promise<{ signalType: string; severity: DetectionSeverity } | null> {
  ...
  private determinePrimaryConstraint(applicant: Applicant): PsnConstraint {
    const risks = [
      {
        type: PsnConstraint.DATA,
        score: this.getDataRisk(applicant.hasInternet),
      },
      {
        type: PsnConstraint.TRANSPORT,
        score: this.getStatusRisk(applicant.currentStatus) * 0.5,
      },
      {
        type: PsnConstraint.TOOLS,
        score: this.getDeviceRisk(applicant.primaryDevice),
      },
    ];
