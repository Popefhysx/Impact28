...
const PHASE_SUPPORT_MAP: Record<ProgramPhase, SupportType[]> = {
  ...
  [ProgramPhase.MARKET_EXPOSURE]: [
    SupportType.DATA,
    SupportType.TRANSPORT,
    SupportType.TOOLS,
  ],
  [ProgramPhase.INCOME_GENERATION]: [
    SupportType.DATA,
    SupportType.TRANSPORT,
    SupportType.TOOLS,
  ...
  async checkEligibility(userId: string): Promise<{
          ...
          where: {
            status: {
              in: [
                ...
                SupportRequestStatus.APPROVED_PENDING_DISBURSE,
              ],
            },
          },
    ...
    if (!user.isActive) {
      return {
        eligible: false,
        reason:
          'Account is paused. Complete a reactivation task to restore access.',
    ...
    const totalMomentum = user.currencyLedger.reduce(
    ...
    if (hasPendingRequest) {
      return {
        eligible: false,
        reason:
          'You have a pending request. Please wait for it to be reviewed.',
  ...
  async createRequest(userId: string, dto: CreateSupportRequestDto) {
    ...
    if (!eligibility.eligible) {
      throw new BadRequestException(
        eligibility.reason || 'Not eligible for support',
      );
    }
    ...
    this.logger.log(
      `Support request created: ${request.id} (${dto.type}) for user ${userId}. Cooldown until ${cooldownUntil.toISOString()}`,
    );
