...
export class AdminService {
  ...
  async getDashboardStats(): Promise<DashboardStats> {
    ...
    const [applicantStats, userStats, incomeStats, missionStats] =
      await Promise.all([
        // Applicant stats
        Promise.all([
  ...
  async makeBulkDecision(
    ...
    this.logger.log(
      `Bulk decision: ${successCount} succeeded, ${failCount} failed`,
    );
  ...
  async decideSupportRequest(
    ...
    const updated = await this.prisma.supportRequest.update({
      ...
      data: {
        ...
        denialReasonCode:
          decision === 'DENY' ? options.denialReasonCode : undefined,
  ...
  private getSupportDecisionMessage(
    decision: 'APPROVE' | 'DENY' | 'COMPLETE',
  ): string {
    switch (decision) {
      case 'APPROVE':
