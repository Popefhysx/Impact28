...
export class AdmissionController {
  ...
  async getPendingTasks(@Param('applicantId') applicantId: string) {
    ...
    if (applicant.status !== 'CONDITIONAL') {
      throw new BadRequestException(
        'Applicant does not have conditional status',
      );
    }
  ...
  async getTaskDetails(@Param('taskId') taskId: string) {
    ...
    > = {
      WHY_STATEMENT: {
        title: 'Why Statement',
        description:
          'Write a 200-word statement explaining why you want to join Cycle 28 and how this opportunity aligns with your goals.',
        instructions: [
      ...
      TIME_AUDIT: {
        ...
        description:
          'Complete a time audit showing how you will dedicate 15+ hours weekly to the program.',
        instructions: [
      ...
      OUTREACH_PROOF: {
        ...
        description:
          'Demonstrate initiative by conducting outreach to 3 potential clients or collaborators.',
        instructions: [
      ...
      INTRO_QUIZ: {
        ...
        description:
          'Complete a short quiz demonstrating your understanding of the Cycle 28 program.',
        instructions: [
  ...
  async submitProof(
    ...
    const proof =
      proofUrl || `text://${Buffer.from(proofText || '').toString('base64')}`;
    ...
    if (!success) {
      throw new BadRequestException(
        'Failed to submit proof. Task may not exist, already completed, or deadline passed.',
      );
    }
    ...
    return {
      success: true,
      message:
        'Task completed successfully. You have been admitted to the program!',
    };
  }
