...
export const RequireCategory = (...categories: ('ADMIN' | 'STAFF' | 'OBSERVER')[]) =>
...
export class ScopeGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        const participantId = request.params?.participantId || request.query?.participantId;
        if (participantId && staff.participantIds.length > 0) {
            if (!staff.participantIds.includes(participantId)) {
                throw new ForbiddenException('Access to this participant is not permitted');
            }
        }
