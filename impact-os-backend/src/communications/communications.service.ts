...
export class CommunicationsService {
  ...
  async retryEmail(id: string): Promise<{ success: boolean; message: string }> {
    ...
    this.logger.warn(
      `Retry requested for email ${id} - full retry not yet implemented`,
    );
  ...
  async queueBulkSend(data: {
    ...
    if (recipients.length === 0) {
      return {
        success: false,
        recipientCount: 0,
        message: 'No recipients match the selected segment',
      };
    }
    ...
    this.logger.log(
      `Bulk send queued: ${recipients.length} recipients by ${data.triggeredBy}`,
    );
