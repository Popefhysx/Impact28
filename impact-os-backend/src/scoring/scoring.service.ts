...
export class ScoringService {
    ...
    async scoreApplicant(applicantId: string): Promise<ScoringResult> {
        ...
        try {
            ...
            const rejectionReason = result.recommendation === 'REJECT'
            ...
            this.logger.log(`Scored applicant ${applicantId}: ${result.recommendation} (${result.readinessScore.toFixed(2)})`);

            // Trigger admission processing (emails, conditional tasks) async
            this.admissionService.processAdmission(applicantId)
                .catch((error) => {
                    this.logger.error(`Admission processing failed for ${applicantId}: ${error.message}`);
                });
    ...
    private scoreWithRules(applicant: Applicant): ScoringResult {
        ...
        // High action signals
        if (technicalProbe.includes('practice') || technicalProbe.includes('try') ||
            technicalProbe.includes('test') || technicalProbe.includes('apply')) {
            actionOrientation = 0.8;
        }
        // Medium signals
        else if (technicalProbe.includes('learn more') || technicalProbe.includes('watch')) {
            actionOrientation = 0.5;
        }
        // Low signals (passive)
        else if (technicalProbe.includes('read about') || technicalProbe.length < 20) {
        ...
        if (commercialProbe.includes('price') || commercialProbe.includes('value') ||
            commercialProbe.includes('market') || commercialProbe.includes('client')) {
            marketAwareness = 0.8;
        } else if (commercialProbe.includes('money') || commercialProbe.includes('pay')) {
            marketAwareness = 0.6;
        ...
        // Resilient signals
        if (exposureProbe.includes('learn') || exposureProbe.includes('improve') ||
            exposureProbe.includes('feedback') || exposureProbe.includes('try again')) {
            rejectionResilience = 0.8;
        }
        // Avoidant signals
        else if (exposureProbe.includes('give up') || exposureProbe.includes('quit') ||
        ...
        if (commitmentProbe.includes('family') || commitmentProbe.includes('future') ||
            commitmentProbe.includes('goal') || commitmentProbe.includes('responsibility')) {
            commitmentSignal = 0.9;
        }
        // Clear purpose signals
        else if (commitmentProbe.includes('need') || commitmentProbe.includes('want') ||
        ...
        if (readinessScore >= 0.70 && riskFlags.length === 0) {
        ...
        return {
            readinessScore,
            scores: { actionOrientation, marketAwareness, rejectionResilience, commitmentSignal },
            riskFlags,
            recommendation,
            diagnosticReport: {
                method: 'RULE_BASED',
                ...
                probeAnalysis: {
                    technical: { text: applicant.technicalProbe, score: actionOrientation },
                    commercial: { text: applicant.commercialProbe, score: marketAwareness },
                    exposure: { text: applicant.exposureProbe, score: rejectionResilience },
                    commitment: { text: applicant.commitmentProbe, score: commitmentSignal },
                },
                resourceFlags: {
...
Return JSON only:
{
  ...
  "reasoning": "Brief explanation"
        ...
        try {
            ...
            const readinessScore =
                (aiResult.actionOrientation * 0.30) +
                (aiResult.marketAwareness * 0.25) +
                (aiResult.rejectionResilience * 0.25) +
                (aiResult.commitmentSignal * 0.20);

            let recommendation: ScoringResult['recommendation'];
            if (readinessScore >= 0.70 && aiResult.riskFlags.length === 0) {
                recommendation = 'ADMIT';
            ...
            return {
                ...
                diagnosticReport: {
                    ...
                    probeAnalysis: {
                        technical: { text: applicant.technicalProbe, score: aiResult.actionOrientation },
                        commercial: { text: applicant.commercialProbe, score: aiResult.marketAwareness },
                        exposure: { text: applicant.exposureProbe, score: aiResult.rejectionResilience },
                        commitment: { text: applicant.commitmentProbe, score: aiResult.commitmentSignal },
                    },
                },
            };
        } catch (error) {
...
Evaluate and return JSON scores.`;
    ...
    private mapRecommendationToStatus(recommendation: ScoringResult['recommendation']): ApplicantStatus {
        switch (recommendation) {
            case 'ADMIT': return ApplicantStatus.ADMITTED;
            case 'CONDITIONAL': return ApplicantStatus.CONDITIONAL;
            case 'WAITLIST': return ApplicantStatus.WAITLIST;
            case 'REJECT': return ApplicantStatus.REJECTED;
        }
    }
    ...
    private determineRejectionReason(result: ScoringResult): RejectionReason {
        ...
        if (result.riskFlags.includes('SHARED_DEVICE') && result.readinessScore < 0.4) {
