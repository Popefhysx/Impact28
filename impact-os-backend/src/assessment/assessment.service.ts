...
const STIPEND_QUALIFYING_STATUSES = [
  'UNEMPLOYED',
  'UNDEREMPLOYED',
  'CAREGIVER',
  'STUDENT',
...
export class AssessmentService {
  ...
  async assessApplicant(applicantId: string): Promise<AssessmentResult> {
    ...
    const receivesStipend = this.determineStipendEligibility(
      applicant,
      offerType,
    );
    ...
    this.logger.log(
      `Assessed applicant ${applicantId}: ${offerType}, stipend=${receivesStipend}`,
    );
  ...
  generateWarnings(applicant: Applicant): string[] {
    ...
    if (
      applicant.currentStatus === CurrentStatus.UNDEREMPLOYED &&
      (!applicant.incomeRange || applicant.incomeRange === IncomeRange.ZERO)
    ) {
    ...
    if (
      applicant.intakeIncomeSource &&
      applicant.intakeIncomeSource !== IntakeIncomeSource.NONE &&
      applicant.intakeIncomeSource !== IntakeIncomeSource.FAMILY &&
      (!applicant.incomeRange || applicant.incomeRange === IncomeRange.ZERO)
    ) {
      warnings.push(
        `Income source is ${applicant.intakeIncomeSource} but claims zero income`,
      );
    }

    // Freelance/Business income source but no online earning attempt
    if (
      (applicant.intakeIncomeSource === IntakeIncomeSource.FREELANCE ||
        applicant.intakeIncomeSource === IntakeIncomeSource.BUSINESS) &&
      !applicant.triedOnlineEarning
    ) {
      warnings.push(
        `Income source is ${applicant.intakeIncomeSource} but claims never tried online earning`,
      );
    }
